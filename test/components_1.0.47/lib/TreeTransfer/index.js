...
exports.TreeTransfer = function (props) {
    var _a = react_1.useState(''), searchValue = _a[0], setSearchValue = _a[1];
    var _b = react_1.useState([]), expandedKeys = _b[0], setExpandedKeys = _b[1];
    var _c = react_1.useState(false), autoExpandParent = _c[0], setAutoExpandParent = _c[1];
    ...
    var generateTree = function (treeNodes, checkedKeys) {
        ...
        return treeNodes.map(function (_a) {
            ...
            var value = index > -1 ? (react_1.default.createElement("span", null,
                beforeStr,
                react_1.default.createElement("span", { style: { color: '#f50' } }, searchValue),
                afterStr)) : (react_1.default.createElement("span", null, title));
            return (react_1.default.createElement(TreeNode, __assign({}, props, { title: value, 
                // @ts-ignore
    ...
    return (react_1.default.createElement(antd_1.Transfer, __assign({}, restProps, { targetKeys: targetKeys, dataSource: flatten(customKey, label, dataSource, []), className: "tree-transfer", render: renderTitle.bind(window, label), onSearch: onSearch }), function (_a) {
        ...
        if (direction === 'left') {
            if (!Array.isArray(dataSource) || (dataSource === null || dataSource === void 0 ? void 0 : dataSource.length) === 0) {
                return react_1.default.createElement(antd_1.Empty, { className: "ant-transfer-list-body-not-found", image: antd_1.Empty.PRESENTED_IMAGE_SIMPLE });
            }
            var checkedKeys = __spreadArrays(selectedKeys, targetKeys);
            return (react_1.default.createElement(antd_1.Tree, { checkable: true, checkStrictly: true, expandedKeys: expandedKeys, autoExpandParent: autoExpandParent, checkedKeys: checkedKeys, onExpand: onExpand, onCheck: handleKeys.bind(window, onItemSelect, checkedKeys), onSelect: handleKeys.bind(window, onItemSelect, checkedKeys) }, generateTree(dataCopy(customKey, label, dataSource, []), targetKeys)));
        }
    }));
};
