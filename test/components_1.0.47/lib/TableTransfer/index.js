...
var useDropdownVIew_1 = __importDefault(require("./useDropdownVIew"));
require("./index.less");
var TableTransfer = function (props) {
    var _a = react_1.useState([]), dataSource = _a[0], setDataSource = _a[1]; // 全部数据 - dataSource
    var _b = react_1.useState([]), targetKeys = _b[0], setTargetKeys = _b[1]; // 右侧穿梭框内的数据
    var _c = react_1.useState([]), sourceSelectedKeys = _c[0], setSourceSelectedKeys = _c[1]; // 左侧穿梭框被勾选的数据
    var _d = react_1.useState([]), targetSelectedKeys = _d[0], setTargetSelectedKeys = _d[1]; // 右侧穿梭框被勾选的数据
    var _e = react_1.useState(1), sourcePage = _e[0], setSourcePage = _e[1]; // 左侧穿梭框当前页码
    var _f = react_1.useState(1), targetPage = _f[0], setTargetPage = _f[1]; // 右侧穿梭框当前页面
    var _g = react_1.useState({ 'left': '', 'right': '' }), filterValue = _g[0], setFilterValue = _g[1]; // 搜索框输入内容
    var _h = react_1.useState(false), showMaxError = _h[0], setShowMaxError = _h[1]; // 错误信息显示状态
    ...
    var LeftDropdown = useDropdownVIew_1.default(handleDropdownConfig('left', "leftDropdown  " + (showSelectAll ? 'TableTransfer-selectAll' : ''))).DropdownView;
    var RightDropdown = useDropdownVIew_1.default(handleDropdownConfig('right', "rightDropdown  " + (showSelectAll ? 'TableTransfer-selectAll' : ''))).DropdownView;
    // 数据转移回调
    ...
    return (react_1.default.createElement("div", { className: "TableTransfer " + className },
        react_1.default.createElement(LeftDropdown, null),
        react_1.default.createElement(RightDropdown, null),
        react_1.default.createElement(antd_1.Transfer, __assign({ dataSource: dataSource, targetKeys: targetKeys, 
            ...
            return (react_1.default.createElement(antd_1.Table, { rowSelection: rowSelection, columns: columns, dataSource: filteredItems, size: "small", style: { pointerEvents: listDisabled ? 'none' : undefined }, onRow: function (_a) {
        ...
        react_1.default.createElement("div", { className: "helpText " + (showMaxError ? "helpTextIn" : "helpTextOut") }, maxErrorMsg !== null && maxErrorMsg !== void 0 ? maxErrorMsg : "\u6700\u591A" + maxTargetKeys + "\u9879")));
};
exports.default = TableTransfer;
